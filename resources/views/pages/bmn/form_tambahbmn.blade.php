...
<div class="row">
	...
	<!-- jquery validation -->
		<div class="card card-primary">
		  ...
		  <!-- /.card-header -->
		  <!-- form start -->
		  <form role="form" id="tambahbmn" method="post" action="{{url('prosestambahbmn')}}" >
		  	{{ csrf_field() }}
			...
			<div class="card-body">
				<div class="row">
				    <div class="col-md-6">
				        ...
				        <label>Kode BMN</label>
				         <select name="aset" id="id_aset" class="form-control select2bs4" style="width: 100%;">
	                        ...
	                        @foreach($aset as $data)
	                            <option value="{{$data->id}}">{{"[".$data->kode_golongan.".".$data->kode_bidang.".".$data->kode_kelompok.".".$data->kode_subkelompok.".".$data->kode."] ".$data->nama}}</option>
	                        @endforeach
	                    </select>
				      </div>
				      <div class="form-group">
				      ...
				      <div class="form-group">
				        <label>Merk</label>
				        <input type="text" name="merk" class="form-control{{ $errors->has('merk') ? ' is-invalid' : '' }}" id="txtMerk" value="{{old('merk') }}" placeholder="Merk">
				      </div>
				      <div class="form-group">
			        ...
			        <div class="form-group">
				      ...
				      <div class="form-group">
				        <label>Jumlah</label>
				        <input type="text" name="jumlah" class="form-control{{ $errors->has('jumlah') ? ' is-invalid' : '' }}" id="txtJumlah" value="{{old('jumlah') }}" placeholder="Jumlah">
				      </div>
						</div>
					</div>
			</div>
