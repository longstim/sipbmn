...
<ol class="breadcrumb float-sm-right">
  ...
  <li class="breadcrumb-item active">Tambah Sub Sub Kelompok BMN</li>
</ol>
@endsection
@section('content')
<div class="row">
	...
	<!-- jquery validation -->
		<div class="card card-primary">
		  ...
		  <!-- /.card-header -->
		  <!-- form start -->
		  <form role="form" id="tambahsubsubkelompokbmn" method="post" action="{{url('prosestambahsubsubkelompokbmn')}}" >
		  	{{ csrf_field() }}
				...
				<div class="row">
				        ...
				        <label>Sub Kelompok BMN</label>
				         <select name="subkelompokbmn" id="id_subkelompokbmn" class="form-control select2bs4" onchange="datasubkelompok(this.value)" style="width: 100%;">
	                        ...
	                        @foreach($subkelompokbmn as $data)
	                            <option value="{{$data->id}}">{{"[".$data->kode_golongan.".".$data->kode_bidang.".".$data->kode_kelompok.".".$data->kode."] ".$data->nama}}</option>
	                        @endforeach
	                    </select>
				      </div>
				      <div class="form-group">
	                ...
	                <div class="col-md-2">
						        ...
						        <input type="text" name="kode_kel" class="form-control" id="txtKodeKel" readonly>
									...
									<div class="col-md-2">
						        <label>Sub Kel.</label>
						        <input type="text" name="kode_subkel" class="form-control" id="txtKodeSubkel" readonly>
	                </div>	
	                <div class="col-md-2">		
						        <label>Sub Sub</label>
				      ...
				      <div class="form-group">
				        ...
				        <input type="text" name="nama" class="form-control{{ $errors->has('nama') ? ' is-invalid' : '' }}" id="txtNama" value="{{old('nama') }}" placeholder="Nama Sub Sub Kelompok BMN">
				      </div>
				      <div class="form-group">
				        <label>Satuan</label>
				        <input type="text" name="satuan" class="form-control{{ $errors->has('satuan') ? ' is-invalid' : '' }}" id="txtSatuan" value="{{old('satuan') }}" placeholder="Satuan">
				      </div>	
				      <div class="form-group">
...
<script>
	$(document).ready(function () {
	  $('#tambahsubsubkelompokbmn').validate({
	    rules: {
	      kode: {
	        required: true
	      },
